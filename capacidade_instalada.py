...
class CapacidadeInstalada(BaseSISPOS):
    ...
    def process (self, f):
        ...
        ipsccm_htot = float(re.search(r'ipsccm.+?([0-9]{1,9}\.[0-9]{2})', f['HTOT']).group(1))
        ...
        o1.write("\tIPCUC\t\t%.2f\n" % (ipcuc_hdisp))
        o1.write("\tIQ\t\t%.2f\n\n" % (icq_hdisp))
        o1.write("\tTOTAL:\t\t%.2f\n\n\n" % (icq_hdisp+ipcuc_hdisp+ipsccm_hdisp))
        ...
        o1.write("\tIPCUC\t\t%.2f\n" % (ipcuc_hefet))
        o1.write("\tIQ\t\t%.2f\n\n" % (icq_hefet))
        o1.write("\tTOTAL:\t\t%.2f\n\n\n" % (icq_hefet+ipcuc_hefet+ipsccm_hefet))
        ...
        o1.write("\tIPCUC\t\t%.2f\n" % (ipcuc_htot))
        o1.write("\tIQ\t\t%.2f\n\n" % (icq_htot))
        o1.write("\tTOTAL:\t\t%.2f\n\n\n" % (icq_htot+ipcuc_htot+ipsccm_htot))
        
        indice_total = ( (icq_hefet+ipcuc_hefet+ipsccm_hefet) / (icq_htot+ipcuc_htot+ipsccm_htot) )*100
        ...
        o2.write("\tIPS + IP-CCM\t%.2f\n" % (ipsccm_htot))
        o2.write("\tIPCUC\t\t%.2f\n" % (ipcuc_htot))
        o2.write("\tTOTAL:\t\t%.2f\n\n\n" % (ipcuc_htot+ipsccm_htot))
        
        indice_dario = ( (ipcuc_hefet+ipsccm_hefet) / (ipcuc_htot+ipsccm_htot) )*100
